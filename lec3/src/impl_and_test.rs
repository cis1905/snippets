...
mod test_traffic_light {
    ...
    fn test_next_light_free() {
        ...
        for (i, status) in statuses.into_iter().enumerate() {
            assert_eq!(
                std::mem::discriminant(&next_light(status)),
                std::mem::discriminant(&next_statuses[(i + 1) % next_statuses.len()])
            )
        }
    }
    ...
    fn test_next_light_if_available() {
        ...
        assert!(matches!(
            TrafficLightStatus::next_light_if_available(
                TrafficLightStatus::Green,
                start - Duration::from_secs(10)
            ),
            Ok(TrafficLightStatus::Yellow)
        ));
        assert!(matches!(
            TrafficLightStatus::next_light_if_available(
                TrafficLightStatus::Green,
